...
class TestInputAligner(unittest.TestCase):
    ...
    def create_msg(self, msg_cls, milliseconds, data):
        stamp = Time(nanoseconds=int(milliseconds * 1e6)).to_msg()
        return msg_cls(stamp=stamp, data=data)

    def test_dispatch_inputs_in_order(self):
        aligner = InputAligner(timeout=Duration(seconds=1.0))
        inputs = [SimpleFilter(), SimpleFilter(), SimpleFilter(), SimpleFilter()]
        aligner.connectInput(inputs)
    ...
    def test_ignores_inactive_inputs(self):
        aligner = InputAligner(timeout=Duration(seconds=1.0))
        inputs = [SimpleFilter(), SimpleFilter(), SimpleFilter()]
        aligner.connectInput(inputs)
        ...
        aligner.dispatchMessages()
        self.assertEqual(self.cb_content, list(range(1, 6)))

    def test_input_timeout(self):
        aligner = InputAligner(timeout=Duration(nanoseconds=int(1e7)))
        inputs = [SimpleFilter(), SimpleFilter()]
        aligner.connectInput(inputs)
    ...
    def test_drops_msgs(self):
        ...
        aligner.dispatchMessages()
        self.assertEqual(self.cb_content, [3, 4, 5])

    def test_dispatch_by_timer(self):
        aligner = InputAligner(timeout=Duration(seconds=1.0))
        inputs = [SimpleFilter(), SimpleFilter()]
        aligner.connectInput(inputs)
        aligner.setupDispatchTimer(self.node, Duration(nanoseconds=int(1e7)))
